...
export default function Footer() {
  ...
  useEffect(() => {
    ...
    const init = () => {
      try {
        if (window.merchantwidget && !document.querySelector('iframe[src*="merchant"]')) {
        ...
        // Widget already initialized, safe to ignore
      }
    };
    ...
    <footer className="bg-surface border-t border-edge/50 mt-auto">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          <div>
            ...
            <div className="flex gap-3 mt-4">
              {info.instagramUrl && (
                <a href={info.instagramUrl} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-inset flex items-center justify-center text-theme-muted hover:text-pink-400 hover:bg-pink-500/10 transition-colors" title="Instagram" aria-label="Instagram">
              ...
              {info.facebookUrl && (
                <a href={info.facebookUrl} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-inset flex items-center justify-center text-theme-muted hover:text-blue-400 hover:bg-blue-500/10 transition-colors" title="Facebook" aria-label="Facebook">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
              )}
              {info.whatsappNumber && (
                <a href={`https://wa.me/${info.whatsappNumber.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-inset flex items-center justify-center text-theme-muted hover:text-green-400 hover:bg-green-500/10 transition-colors" title="WhatsApp" aria-label="WhatsApp">
          ...
          <div>
            <h4 className="text-sm font-semibold text-theme-secondary mb-3">Company</h4>
            <div className="space-y-2">
        ...
        <div className="border-t border-edge/50 mt-8 pt-6 text-center">
          <p className="text-xs text-theme-dim">&copy; {new Date().getFullYear()} {info.platformName || 'Giftsity'}. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
